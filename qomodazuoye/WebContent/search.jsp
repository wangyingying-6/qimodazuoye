...
<%
	...
	//获取总页数
	pageCount = gc.getDaohlGoodsCount(pageSize,tj);	
	//获取到商品总数
	int goodsCount =gc.getSumGoodsCount(tj);
	//如果小于第一页，则显示第一页
	if(pageNow <= 0){
		pageNow = 1;
	}
	...
	ArrayList<GoodsInfo> agoods = gc.getDaohlGoods(pageSize,pageNow,tj);
		if(agoods==null){
...
<html>
  <head>
	...
	<style type="text/css">
		...
		.gg,.sortpic{position:absolute; left:4%;width:950px;height:130px; top:950px; border-top:#D0D0D0 0px solid; border-bottom:#D0D0D0 0px solid;}
		/*==显示搜索商品==*/
		.seh{width:930px; border-left:#D0D0D0 1px solid; border-right:#D0D0D0 1px solid; margin:0px auto; height:37px; line-height:20px;}
		.seh,.sortpic{position:absolute; left:28.6%;width:625px;height:770px; top:180px; border-top:#D0D0D0 1px solid; border-bottom:#D0D0D0 1px solid;}
  ...
  <script type="text/javascript">
  	function pageGo(){
		...
		if(isNaN(pageNow.value)){
  		...
  		window.location.href="search.jsp?pageNow=" + pageNow.value+"&tj=<%=tj %>";
  	}
  </script>
  <body>
  	<!-- 总层 -->
    <div id="sum">
    	...
    	<div id="search" class="seh">
			...
			<tr style="position: absolute;top: 735px;height: 40px;">       			
       			<td colspan="5">
       				<center>
       				<a href="search.jsp?pageNow=1&tj=<%=tj %>"><img src="images/first.gif"></img></a>&nbsp;&nbsp;
       				<a href="search.jsp?pageNow=<%=pageNow-1 %>&tj=<%=tj %>"><img src="images/back.gif"></img></a>
       				<font style="color: #8E8E8E; font-size: 12px;"><%=pageNow %>/<%=pageCount %></font>
       				<a href="search.jsp?pageNow=<%=pageNow+1 %>&tj=<%=tj %>"><img src="images/next.gif"></img></a>&nbsp;&nbsp;
       				<a href="search.jsp?pageNow=<%=pageCount %>&tj=<%=tj %>"><img src="images/last.gif"></img></a>
